...
function printReceipt($saleId, $method = 'normal') {
    ...
    $bridgeResult = attemptBridgePrint($result['file'], $saleId);
    $response['printed'] = $bridgeResult['printed'];
...
// ============================================
// Dispatches to the configured print bridge and ALWAYS returns a reason —
// 'printed' is only null when nothing is configured to attempt at all.
// Used by both printReceipt() (real sales) and the Settings "Test Print"
// button (test_print ajax action), so the exact same logic is exercised
// whether you're debugging config or actually printing a sale.
// ============================================
function attemptBridgePrint($pdfFilePath, $saleId = null) {
    ...
    if ($printerMethod === 'pdf') {
        ...
        if ($sumatraPath === '') {
            return ['printed' => false, 'message' => 'SumatraPDF Path is empty in Settings → Printer Configuration.'];
        }
        if ($printerName === '') {
            return ['printed' => false, 'message' => 'No Windows Printer Name selected in Settings → Printer Configuration.'];
        }
        $r = printViaSumatraPdf($pdfFilePath, $sumatraPath, $printerName);
        return ['printed' => $r['success'], 'message' => $r['message']];
    }

    if ($printerMethod === 'windows') {
        $bridgePath = trim($settings['printer_bridge_path'] ?? '');
        if ($bridgePath === '') {
            return ['printed' => false, 'message' => 'Bridge Executable Path is empty in Settings → Printer Configuration.'];
        }
        ...
        if ($saleId === null) {
            return ['printed' => false, 'message' => 'The "Windows Drivers" text method needs a real sale to print — use "Print Exact PDF" to test without one.'];
        }
        $r = printViaTextPrinterBridge($saleId, $bridgePath, $printerName);
        return ['printed' => $r['success'], 'message' => $r['message']];
    }

    if ($printerMethod === 'network') {
        return ['printed' => false, 'message' => 'Network direct printing is not implemented yet — switch Printing Method to "Print Exact PDF".'];
    }

    return ['printed' => false, 'message' => "Unrecognized printer_method value: \"$printerMethod\"."];
}
...
function printViaSumatraPdf($pdfFilePath, $sumatraPath, $printerName) {
    ...
    $errorDetail = trim(implode("\n", $outputLines));
    return ['success' => false, 'message' => $errorDetail !== '' ? $errorDetail : "SumatraPDF exited with code $exitCode. Command: $cmd"];
}
...
// ============================================
// Standalone test receipt — used by the Settings "Test Print" button so
// printer setup can be verified without needing a real sale to exist.
// Includes Arabic content on purpose, since that's the whole point.
// ============================================
function generateTestReceiptPDF() {
    ...
    $html = '<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><style>
        body { font-family: dejavusans, Arial; font-size: 13px; text-align: center; padding: 6px; }
        .title { font-size: 1.4em; font-weight: bold; margin-bottom: 6px; }
        .line { border-top: 1px dashed #333; margin: 8px 0; }
        .ar { font-size: 1.1em; margin: 4px 0; }
    </style></head><body>
    ...
    $pdfFile = $receiptDir . '/test_print_' . date('Ymd_His') . '.pdf';
...
function generateReceiptPDF($saleId, $method = 'normal') {
    ...
    $mpdf = new \Mpdf\Mpdf([
        'mode'          => 'utf-8',
        'format'        => [82, 150], // thermal receipt size
        'margin_top'    => 5,
        'margin_bottom' => 5,
        'margin_left'   => 1,
        'margin_right'  => 1,
